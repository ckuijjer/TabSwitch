$(function () {
  chrome.tabs.query({ currentWindow: true }, function (tabArray) {
    ...
    chrome.runtime.sendMessage(
      { command: 'getMostRecentOrder', arguments: { windowId } },
      function ({ mostRecentOrder }) {
        // use the most recent order from the background page to order the tabs
        tabArray.sort(function (a, b) {
          var indexA = mostRecentOrder.indexOf(a.id);
          var indexB = mostRecentOrder.indexOf(b.id);
        ...
        tabArray.forEach(function (tab) {
          ...
          $('<option>').val(tab.id).text(tab.title).appendTo($select);
        });
